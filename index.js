...
function indekseGoreFenomen(a, b) {
  return b + ". indekste bulunan fenomen: " + Object.values(a)[b].profile;
}
...
function profilListesi(a) {
  const yeniDizi = a;
  const profileDizi = yeniDizi.map((items) => items.profile);
  return profileDizi;
}
...
function fenomenEkle(a,b,c,d,e,f) {
  var copyFenomenler = a;
  var yeniData = {
    "number": b,
    "profile": c,
    "followers": d,
  ...
  copyFenomenler = [...copyFenomenler,yeniData]
...
function platformaGoreCokGonderiYapanFenomen(a,b) {
  let enCokGönderiProfili="";
  let enCokGönderiAdedi=0;
  for(var i = 0 ; i<a.length ; i++){
    if(a[i].platform === b && a[i].posts !== "" & a[i].posts > enCokGönderiAdedi){
